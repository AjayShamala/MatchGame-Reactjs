...
class App extends Component {
  ...
  componentDidMount() {
    this.timerId = setInterval(this.matchInterval, 1000)
  }
  matchInterval = () => {
  ...
  imageClick = thumbnailUrl => {
    ...
    const newImgUrl = imagesList.filter(
      each => each.thumbnailUrl === thumbnailUrl,
    )
    const {imageUrl} = newImgUrl[0]
    if (imageUrl === imgUrl) {
      const newList =
  ...
  render() {
    const {isTrue, score, category, imgUrl, time} = this.state
      ...
      <div className="bg-container">
        <nav className="nav-container">
          ...
          <ul className="containerssss">
            <li className="list">
              <p className="para">
                Score: <span className="span-container">{score}</span>
              </p>
              <img
        ...
        <div className="containers">
          {!isTrue && (
            <div className="div-container">
              <img src={imgUrl} alt="match" className="image-4" />
              <ul className="unorder">
                {tabsList.map(each => (
                  <li className="class" key={each.tabId}>
                    <button
                      className={`buttons ${
                        category === each.tabId ? 'border-color' : ''
                      }`}
                      onClick={() => this.onClickTabId(each.tabId)}
                    >
                      {each.displayText}
                    </button>
                  </li>
          ...
          {isTrue && (
            <div className="gamee-container">
              <img
                src="https://assets.ccbp.in/frontend/react-js/match-game-trophy.png"
                alt="trophy"
                className="image-6"
              />
              <p className="para9">Your Score</p>
              <p className="para8">{score}</p>
              <button className="buttons-play" onClick={this.playAgain}>
                <img
